...
⚠️ **⑤ 跨租户可见性：当前「没有强制边界」—— 如实登记，待架构裁决（2026-09-20）。**

- **实测**（W7 2026-09-19）：`app.audit_log` 的 `relrowsecurity = f`、**0 条策略**；
  而 6 张业务底表是 `rls = t` **且** `force = t` 各挂 1 条策略。
  所以"**RLS 是最终强制边界**"这句话**对本表不成立** —— 本表的边界只有上面 ① 权限 + ② 触发器，
  而这两者管的是**不可变（不许改/删）**，**不是**"跨租户不可见"。
- **现状的准确描述**（不是"两者都不是"，而是这一种）：`app_rw` 有 `SELECT`（迁移 `0001` 授予）、
  且无 RLS ⇒ **跨租户可见**；除属主 `app_rw` 与超级用户外无其他角色可读
  （`0001` 已 `REVOKE ALL ... FROM PUBLIC`）。**且代码里没有读路径** ——
  本文件只有 `write_pre` / `write_supp` ⇒ 今天**不可达**，不是"已暴露"。
- ⚠️ **补策略有个会打断全站的陷阱，裁决前必须知道**：PG 的 RLS 是**默认拒绝** ——
  开了 RLS 却只写一条 `FOR SELECT` 的租户策略时，**`INSERT` 会被拒**（无适用策略 = 拒），
  而审计写入是 **fail-closed**（NFR-3.4）⇒ 后果是**每个请求都失败**（§RL-1 的触发形态）。
