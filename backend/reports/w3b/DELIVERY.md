...
## 八、【勘误·2026-09-17 傍晚】本报告两处报数被 W3A 修正（**收口请以本节为准**）

> 触发：W3A `d89dc07`+`e582fe0`（阶段预算解耦）与 `aac3091`+`672fdf4`（思考档 max_tokens 标定），证据 = `reports/w3a/DELIVERY.md §12/§13`。
> 以下两条**本窗口报窄了/报反了**。不悄悄改正文——原文保留在上面，供核对勘误链。

### 勘误 1：`normalize_intent` "5/5 通过"是悬崖边样本；受影响范围是 **8 个 task**，不是 2 个

- 本报告 §二 DoD① 表报 `normalize_intent` 5/5（中位 939ms）。**W3A 直连网关同口径实测 0/3 失败**（真机 1.2–1.6s，恰卡 1.2s deadline）——它正踩在悬崖边，本窗口的 5 个样本恰好全部落在"通过"一侧。
- ⇒ "**只有 plan/gen_sql 失败**"是**低估**：凡有 §16.1 分配的 task **全部中招**——`normalize` 0/3、`intent` 0/3、`normalize_intent` 0/3、`plan` 0/3、`gen_sql` 0/3；`rerank`(0.8s)/`l4_score`(0.8s) 同罩此坑未单列实测，`present`(1.5s) 未实测。**双向对照 = 0/15 → 15/15**（失败耗时恰好等于各自预算值 ⇒ 是"卡在 deadline 上死"，不是上游不可用）。
- 结论修正：这不是"两个 task 的配置错"，是**"把 P95 分位数当硬超时"的逻辑错误**（分位数当上限 ⇒ 天生砍掉健康调用的 5%；分配值 < 真实中位 ⇒ 100% 失败），影响**全部**有分配的 task。

### 勘误 2：`gen_sql_complex` 不是"2/3 空 content"，是 **3/3 确定性失败**；且 pro 档端到端**从未生效**

- W3A 用真实 L3+ 问句（带环比+品类排名）直发上游 n=3：`max_tokens=3248` 三次**全部** `finish_reason=length` + 空 content，`reasoning_tokens` 恰好吃满 3248。本窗口报的"1 次成功（41130ms）"是**更简问句下的偶然**，不作基线。
- **已标定**（W3A，`router.py` 内部常量，W3B 无需改任何调用代码）：`THINKING_HEADROOM_TOKENS` 2048→**8192**；`GEN_SQL_COMPLEX.output_tokens_hint` 1200→**1536**（旧值低于实测 content 需求 1223）。
- 🔴 更重的结论：**pro 档在真机从未生效**——端到端 n=2 **每次**白等 45s 被传输层掐断（`LlmTimeout`）→ 降级 flash，**50–60s/请求**。即 **PRD §12.2（L3+ 走 pro 思考）× 07 §10.2（pro 45s）× NFR-1.2（8s P95）三者不可同时成立**（pro 真实需 97–138s），已上呈架构候选 **⑬**（A/B/C 三方向，W3A 不预设立场）。**别对 pro 档质量做任何依赖，也别在文案/日志里承诺它。**

### 连带影响本窗口交付的两条

1. **§六 复现指引里的三个探针脚本已全部失效**：它们 monkeypatch 的 `effective_timeout_s` 已被 W3A **删除**——`TaskRoute.timeout_s` 改名 **`budget_s`**（§16.1 阶段分配，**纯元数据，不参与任何超时判定**，只驱动 `CallRecord.over_budget` 观测）；真实 deadline 一律 **`hard_timeout_s(model_key)`** = §10.2 模型级上限（flash 15s / pro 45s），**刻意不看 task**。§二 DoD① 表的"旁路"列**不再可复现**，仅作历史证据。
2. **DoD① 判定不变，数值口径让位**：W3A 直连网关口径 `plan` 1.49s / `gen_sql` 1.60s / `normalize_intent` 1.56s（vs 本窗口经 `PlannerEngine` 的 2756/1939/794ms——差异 = 语义包 + prompt 组装）。**数值以 W3A §12.1 表为准**；两个口径结论一致：**全部超 §16.1 分配**。
3. **修复后行为变化（给后续窗口）**：超阶段预算**不会再失败**，而是照跑完 + `over_budget=true`；**超预算不发 `degraded` 事件**（刻意——降级事件的含义是"答案来自降级路径"，延迟超标不改变答案）。§16.1 的执行责任移交 W4 的 SSE（推占位符）。
