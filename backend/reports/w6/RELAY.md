...
| P7 ★ | 你 ④ 报的"并行计划把 `count(*)` 吃掉一截"——**我按能想到的形状复现不出来，请给复现条件** | 我的复算（只读、每次独立新连接、`app_ro` + `set_config` 两把 GUC、`v_order_paid` 200,000 / `v_traffic_daily` 600,178）：`debug_parallel_query` off/on × `is_local` false/true（后者与你的 SET ROLE 形状更接近事务内注入）× 各 5 次 = **8 组全部读数精确且一致**，并且 `EXPLAIN` 里确有 `Gather`/`Parallel Index Only Scan` ⇒ 并行路径**真的被走到**了，不是"没并行所以相等"。我也没能复现你给的 98,359 / 98,052 / 1,271 这一族数（更像"部分并行分量为 0"的形状）。请给我四件事之一即可定位：① 你当时是**经 app 执行器**（服务端命名游标 + `EXEC_MAX_ROWS` + 取消路径）还是裸 SQL；② `SET ROLE app_rw` 与直接以 `app_rw` 登录的差别；③ 那条 SQL 的确切形状（是否带 GROUP BY / 游标批量）；④ `max_parallel_workers_per_gather` 当时是多少。**不管复现不复现，这条已经变成我的前置判据**：`_probe_pg_real.py` 现在常驻 `parallel_equality`（off/on 各 3 次 + 计划里必须真有并行节点 + `equal` 判定），PG 执行链接进评测前它必须先绿 —— 因为 N-07 抓不到这种错（策略照样生效、不报错、只是数变小）|
...
6b. **PG 适配器的两条硬要求**（从 P6/P7 长出来的，不是可选项）：① 连接装配必须同时设 `app.tenant_id` **和** `app.shop_ids`（只设前者 ⇒ 三张表恒 0 行且不报错，评测会把环境的沉默读成模型答错）；② 每批结果附一次"并行开/关结果相等"对照（`_probe_pg_real.py:parallel_equality` 已经给了形状），先绿再接 G-4。
...
| 我第一版并行对照给出 `count(*) = 0`、`sum = None`，看起来正好**复现**了 W7 的 ④ | 是我自己的实验缺陷：同一条连接、同一个事务里反复改 `debug_parallel_query` ⇒ 读的不是并行路径。每例独立连接重写后 8 组全部精确 | 按"未能复现"上报（P7），并把这项对照常驻进探针 + 三态措辞派生（`gap_table.parallel_clause`）；教训：**"抓到的异常"要先排除是自己造出来的** |
