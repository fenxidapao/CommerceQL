...
| P9 ★ → W4 | 你们的接缝契约 `8a4121a` **输入形状可能选错了**，修完 U-121 它仍会红 | `tests/contract/test_gate_seam_contract.py:54` 把**扁平面** `asset_allowlist()` 原样喂 `run_gate1` 并要求 pass。我方实测：扁平面 → `R05` 拒（**任何** SQL 都拒，因为拿不到 `assets` 键）；端口 `guard_allowlist` → 两条 SQL 都 passed。⇒ 按 U-121 的正解（消费方改调 `guard_allowlist`）改完之后这条**照样红**，因为它喂的还是扁平面；它唯一的绿路径是"给扁平面补 wrapper 键"—— 而那正是 07 与 `contracts.py:423` 明令禁止的做法。**请二选一**：① 把判据①的输入改成端口形状（判据②的反向对照保留）；② 保留输入，但在断言或文档里写明"这条红的是**消费侧未接线**、不是形状本身"。我方不替你们改（`tests/contract/**` 归 W4）。复算：`cd backend && PYTHONUTF8=1 ../.venv/Scripts/python.exe -m pytest tests/contract/test_gate_seam_contract.py -q -rf` |
...
| O6 ★ | **同一条夹具根因今天又长出了 3 条 `ERROR`**（第四轮复测，请并进 O5 一起处理） | 复算：`cd backend && PYTHONUTF8=1 ../.venv/Scripts/python.exe -m pytest -q` → **1 failed / 2222 passed / 6 skipped / 3 errors（112.70s）**。3 条 error 全在 `tests/integration/test_retrieval_fts_pg.py` 的 `dense_*`（夹具 `dense_table` 里 `CREATE SCHEMA` 抛 `InsufficientPrivilege: permission denied for database ecom`）⇒ 与 O5 那 6 条 skip **同一根因**，只是那条路径有 `try/except → pytest.skip`、这条没有 ⇒ 表现成"错误"而不是"跳过"。⚠️ 后果：**G-1 从 PASS 落成 FAIL**（`failed = 1 failed + 3 errors`），而两条红因都不在评测窗口内 ⇒ 请裁决时把它们与"真回归"分开数；我方已让 `gates._p0_notes()` 在报告里**逐条点名到测试**，不再只报一个 `failed=N`。归属：夹具 = W0/W2B（`c6373a2`/`1255065` 那条 dense 路径） |
