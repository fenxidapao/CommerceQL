...
| **第九轮预检（README §三.0.1j = 本窗口第十三轮；09-22 19:48 · 镜像 `w7load-api:0922r6` = `fb8b4c6`）：闸门通了，判据④ 仍未过 —— 红因换成一条"两边都合规的死锁"** | 前置三查（含 `embed_doc` 跑前跑后）→ 亲跑 W2C 器件与 redteam 全链复现 → 重建镜像 → 先报规模与花费 → c=1/n=5 → `/metrics` + `docker logs` + `app.audit_log` 只读 + 两支新器件（离线闸门面 7 题面、生产池六臂 A/B） | ★ **`stage_duration_seconds_count{executing}` 第一次非 0**（`schema_linking/plan_ready/sql_ready` 各 1、`gate_passed` 仍 0）、`query_outcome_total{failed}=1` ⇒ **链路第一次死在执行面**。❌ 但 `ok=0` ⇒ **判据④ 未过 ⇒ 四场景仍不跑批**。根因（三条独立实测）：**闸门只认非限定资产名**（`app.v_order_paid` ⇒ gate1 `R16`，`ast_gate.py:531-534`），而 **analytics 池 `search_path="$user", public` 解析不了任何非限定名**（六臂 A：`relation "v_order_paid" does not exist`；B/C：加上 `search_path=app` 即 `count=200000`）⇒ **能过闸门的一定解析不了**。★ 附带定性：E 臂证明 `gate3_explain_failed(EXPLAIN 不可用→warn)` 与 `exec_failed(unknown_table)` **同因** ⇒ 记一笔不记两笔。⚠️ 归属互相指认（`pools.py:227` 写"归 W2A"、`dsn.py:141` 身份模板不带）⇒ **需求上报、不自取号**（下一可用号 U-124）。花费：**9 次调用 / ¥0.014471**（跑前报备 ¥0.02–0.04 ⇒ 声明偏保守；`httpx 200` 9 = `llm_call` 9 零缺口；其中 `repair` 单跳 **¥0.006592**、`cache_hit=0`，产出是一条被 `R05` 拒的裸表 SQL = 纯浪费）|
| **第十轮预检（README §三.0.1k = 本窗口第十四轮；09-23 16:0x · 镜像 `w7load-api:0923r7` = `776beb4`，含 W2A `d8eca02` = U-124）：★ 判据④ 第一次达成** | 前置三查 → **六臂件 A/E 两臂独立验收 U-124**（不复述 W2A 回执）→ 先报规模与花费 → 与上轮**逐参数相同**的 c=1/n=5 → `/metrics` + `app.cost_ledger` 结账 + `embed_doc` 跑前跑后 | ★ `outcomes={ok:3, clarify:1, refuse:1}`、**`codes={}`**（九轮以来首次零错误帧）、`stage_duration_seconds_count` 六档全亮：`intent 6 / schema_linking 3 / plan_ready 3 / sql_ready 3 / **gate_passed 3** / **executing 3**`、`query_outcome_total{success}=3` ⇒ **判据④ 达成、四场景前置解除**。U-124 验收：A 臂生产池原样 `count=200000 search_path=app` + `EXPLAIN` 出计划（上轮同一条命令是 `relation does not exist`）。⚠️ 但 `admitted=5 < 20` ⇒ `g6_p95_le_8s=null`（caveat 只剩"样本不足"一条）⇒ **G-6 仍不得宣称**。花费 **14 次 / 62,552 tokens / ¥0.030397** —— ⚠️ **超我报备上限 ¥0.025**：锚点错在"拿失败路径的单价外推"，**一条 `ok` ≈ 4 次调用 ≈ ¥0.0087，成功比失败贵 3–5 倍**（已写进 `HANDOFF §六` 与 §三.0.1k，四场景报价改用新锚点） |
| **🔻 我撤回一条自己写下的取证口径：「PG 重启 ⇒ 统计清零」**（09-23，W2B 反驳 + 我复测确认） | `pg_stat_database.stats_reset`、`pg_stat_user_tables.n_tup_*` ×两时刻、`pg_postmaster_start_time()`、`docker inspect RestartCount`（**全部只读**） | ❌ 我 09-22 那句"整栈 19:18 重启 ⇒ 统计清零 ⇒ `10244\|10244` 是 25 分钟内 52 次重灌"**判错**：PG **干净重启保留** `pg_stat` 统计（落盘/回载），`stats_reset=NULL` 只表示"没人显式 reset 过"、**不给起点日期**。★ **决定性对照**：PG 在 09-23 07:55:59Z 又干净重启一次（`RestartCount=0`、`created` 未变），而计数器**仍是 `10244\|10244` 一格没动** ⇒ 10244 是**累计量** ⇒ ① 我把绝对值当速率用了（同族错误：`stage 计数=帧数、不能做减法`）；② 我说"§三十取证链无法续接"也判反了 —— 累计量**可比**，差分读数 = 15:17→19:43 **21 次**、19:43→次日 15:57 **0 次**。⇒ 本窗纪律改为**只用带时刻的两读差分**（`HANDOFF §六` 已落该条 + 复测命令）。⚠️ 同时登记：共享栈两次被外部重启（09-22 19:18、09-23 15:55）、我昨日容器 `Exited (137)` —— **均非我所为，退出原因我没查不猜** |
| 🔻 **09-22 19:43 前置复测（本行"统计清零"那句推断已被上一行订正，原文照留不改）** | 只读 `count(*),count(embedding),count(tsv)` ×2（跑前/跑后）+ `docker inspect .State.StartedAt` + `pg_postmaster_start_time()` + `pg_stat_user_tables` + `git diff --cached --name-only` | ✅ `197\|197\|197`（跑前 19:43、跑后 20:08 两次同值）⇒ W2B `d4ca203` 的重灌 + 两道防线（W2A `5e47558` U-123/防线①、W0 `36c782a` 防线②）成立。**新事实**：四条容器 `started=11:18:17~18Z`（**19:18 本地**）而 `created` 未变 ⇒ 是**重启不是重建**，`pg_postmaster_start_time()` 同刻 ⇒ ① 统计视图清零 ⇒ **§三十那条 14:26–15:06 取证链无法向后续接**；② 重启后 25 分钟内 `embed_doc` `ins\|del` = `10244\|10244` = **52×197**（形态与"整表删后重灌 ×52"一致，**不指名成因**；我本轮只跑纯 AST 的 `test_harness_allowlist.py` = 27 passed，未跑任何会重灌的套件）。共享索引：`git diff --cached` **空**、工作区只见别人的 2 个 `M`（W6 两支探针）⇒ 不 stage 不还原不删除 |
| **第十一轮（README §三.0.1l = 本窗口第十五轮；09-23 晚 · 镜像 `w7load-api:0923r8` = `f5e501d`，含 W2B 的 RL-2 接线）：★ `retrieval_mode_total` 第一次有活体序列 + 判据④ 在新镜像复现** | 前置（`healthz` 7 项全 true、`embed_doc` 跑前跑后）→ 先报规模与花费 → 4 次运行（冷 `c1n2` / 热 `c1n2` / `session-lock c8n24` / `steady c1n5`）→ `/metrics` 三点差分 + `docker logs` + `app.cost_ledger` 只读结账 | ★ **`retrieval_mode_total{hybrid}=2`**（同容器基线 0）⇒ **W2B `f5e501d` 接线由我独立复验成立**；⚠️ `sparse_only` 仍 0 ⇒ 降级分支 **UNVERIFIED**（我不为它制造降级）。★ **判据④ 复现**：`outcomes={ok 2, clarify 2, refuse 1}`、`codes={}`、六档 stage 同亮（`gate_passed 2` / `executing 2`）⇒ **走通到执行不是一次性事件**；`admitted=5 < 20` ⇒ `g6_p95_le_8s=null`、**G-6 仍不得宣称**。★ **本仓第一条干净冷/热单变量对照**：同镜像同题同 `n=2` 只差容器冷热 ⇒ **p95 72,768.7ms → 1,345.7ms（54 倍）**，§三.0.1i 那条"未做对照 ⇒ 不写成成因"的限定**撤销**。★ `session-lock`：`admitted=1` + **9×409 `SESSION_CONFLICT`(RA 3)** + **14×429 `RATE_LIMITED`(RA 30)** ⇒ 与 `ratelimit.py:203` `QUERY=(per_user 10/min)` 的算术**逐格对上**（24=10+14；10=1+9）⇒ **§16.5 场景③ 的参数把被测对象换成了限流器**，锁只拿到 10/24 观察面（**我不擅改契约面，提架构裁**）。★ **免费自洽断言**：`degraded{present_failed}=2 == outcome{success}=2`（`deps.py:824-826` 的 P0 既定形态）⇒ W3B 接上 `present` 时会断，届时别当回归。花费 **13 次 / ¥0.013110**（预估 ≤¥0.07 ⇒ **低于预估的原因是准入数低、不是单价准**） |
| 🔻 **我撤回一条本轮自己刚产生的误读：「判据④ 不可稳定复现」**（09-23 晚，**写进任何对外结论之前就自我否证**） | 读 `deploy/loadtest/driver.py:280` + 对比 `questions_T_A_time.txt` 题序 + `preflight_r7.json` 的 `terminal_provenance` | ❌ 起因：`n=2` 两次都 0 条 `ok`，我判成回归。真相 = 取题是 `questions[i % len]` ⇒ **`n=2` 只打到第 0、1 题**（`日期维表` → `intent refuse(out_of_scope)`；`从 2026-08-01 起` → `clarify(time_ambiguous)`），**这两题结构上到不了 `link`**，r7 的 3 条 `ok` 来自 Q2–Q4 ⇒ **不是链路回归，是我拿"换了题集前缀"的两条样本回答了一个回归问题**。⇒ 归档件 `preflight_r8_cold/warm.json` **保留当反例**；纪律落成 HANDOFF §六 一条：**跨轮比较必须复刻 `(n, questions-file, concurrency)` 三元组**。⚠️ 连带订正我自己 §三.0.1g 的"三族没有调用点 ⇒ 死路"：真实成因是**请求没走到那段代码**（`binding_*` 一直在 `api/deps.py:563/564`、`retrieval_mode` 现由 W2B 补上）⇒ **"指标恒 0"有两种成因、修法完全相反**（换题面 vs 改代码） |
| 🔴 **共享前置第三次外部重启 + `pg_stat` 差分口径的基线断裂**（09-23 12:47:19Z，**非我所为，全程只读**） | `pg_postmaster_start_time()` + `pg_stat_user_tables.n_live_tup/n_tup_ins/upd/del` + `count(*)` + `count(embedding)/count(tsv)` + `pg_stat_database.stats_reset` | ⚠️ `embed_doc`/`cost_ledger`/`query_plan`/`audit_log` 四张表的**累计计数全部归零**，而数据完好（`197 / 23 / 4 / 108`）⇒ **归零后无法区分"别人重灌 N 次"与"统计被清"**，我 §三十二② 刚立的"只用带时刻差分"口径**当场失去基线**，U-114 在 12:47Z 之前的取证链**永久不可重建**。⇒ 前置判据退回 **`count(*)=197` + `count(embedding)/count(tsv)` 快照**（本轮跑前跑后各一次，均 `197\|197\|197` ⇒ **我没损坏共享前置**）。⚠️ **成因我不写**：`pg_stat_database.stats_reset` 对 `ecom` 仍是 NULL（与"`pg_stat_reset()` 被调用"不同态），且我没有单变量对照。★ 通用教训已进 HANDOFF §六：**任何"累计量差分"结论都要附一条"如果计数器归零，这条结论怎样失效"** |
...
| `retrieval_mode_total` 的降级率告警口径 | 指标**无调用点** ⇒ 比值分母为 0，当前只能靠 `/healthz` 的 `degraded_dependencies` + 日志判 RL-2★ **09-21 有了决定性对照**：同一轮预检里 `sparse_only` 降级**真实发生**且 `degraded_total{reason="embedding_unavailable",action_taken="sparse_only"}=1`，而 `retrieval_mode_total{retrieval_mode="sparse_only"}` **仍 = 0** ⇒ **"无调用点"不是数据假象，是缺埋点**（缺的是 `link` 侧的计数调用，不是流量）。⇒ RL-2 的分子改建于 `degraded_total`（现可用），埋点需求提给拥有 `link` mode 落点的一方；另记一条口径陷阱：`query_outcome_total{outcome="degraded"}=0` 而两条请求确实"既降级又被拒"（终止态只记 `refuse`）⇒ **看板别拿 `outcome="degraded"` 当降级率分母**，会系统性低估。 ★ **09-23 晚订正（本行的"缺埋点"已被活体读数推翻）**：W2B `f5e501d` 已补 `link` 侧调用点，我在 `0923r8` 上实测 **`retrieval_mode_total{hybrid}=2`**（同容器基线 0）⇒ 该族**第一次有活体非 0 序列**。**"恒 0 有两种成因"这条通用教训留在本行**（换题面 vs 改代码，修法相反），见 `README §三.0.1l`。 |
| ~~RL-2 降级率口径欠账~~ ✅ **已还（09-22 第十轮）** → ★ **09-23 晚升级为「分子可用、分母待定」** | 四处同步改挂 `degraded_total`：`RL-2` §22/§132、`runbook/README.md` 索引行、`alert.rules.yml` 末尾注释、看板 `description`；两个假分母（`retrieval_mode_total` / `outcome="degraded"`）已逐处点名禁用。`promtool check rules` + `test rules` 复跑 SUCCESS。~~**剩下的只有"接线"这一件，在 W2B**~~ ⇒ ✅ **W2B 已接（`f5e501d`）、我已独立复验**（`hybrid=2`）。⚠️ **剩余两格不是"接线"了**：① `sparse_only` 分支 **UNVERIFIED**（本轮无降级发生，我不制造降级）；② **分母口径未定** —— 比率要的是"经过 `link` 的**请求数**"，而 `stage_duration_seconds_count{stage="schema_linking"}` 是**节点执行次数**（§三.0.1g 血案同款），**不得当分母**。⇒ 在 ② 定案前，RL-2 仍**不得作为达标证据引用**（但"缺埋点"这条已不再是理由）。 |
...
· 本窗口**没有**改动共享栈：全程只做只读 `curl` / `docker exec` 与一次性容器，`docker ps` 三容器 Up 时长未断。
  ★ **09-22 晚订正这两句的现时有效性**：① 共享栈四条容器在 **19:18:17~18Z（本地 19:18）被重启过**（`created` 未变 ⇒ 重启非重建），
  **动作不是我做的**，但"Up 时长未断"从此**不再是我能用来证明"谁都没动过栈"的证据** —— 它只能证明"没人重建容器"。
  ② 本窗口今天第一次在共享 `ecom` 上跑过 **只读 `SELECT` / `EXPLAIN`**（经生产 `build_analytics_engine`，身份 `app_ro`，
  六臂 A/B，见 `scratch_searchpath_ab.py`）⇒ 口径收窄为"**只读、不写不删不改 DDL**"，而不是"没碰库"。
  ③ 禁令照守：未跑 `tests/integration`、未跑迁移套件、未重启/重建任何共享容器、未改别人的文件。
