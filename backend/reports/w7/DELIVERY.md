...
| **R-17 未解决风险：`RESET` 占位符 GUC 在并行计划下静默少算**（`U-110`，机制已定位、**修复不在我手里**） | 同会话·同 REPEATABLE READ 快照·每轮只切换并行度，4 轮全复现：**串行恒 200,000 / 并行 119,181、104,708、93,019、115,622（每次不同、全部偏小、无报错）**，属主真值同轮取。按进程证据：`Worker 0: actual rows=0`（占位符）vs `Worker 0: actual rows=92,983`（显式设值）。⇒ 占位符不进 worker、RLS 那一支含逐行比较**无法提升成 `One-Time Filter`**、并行索引扫描**动态派块**三条合起来就是"存活量 = leader 随机抢到的一份"。**已提请架构与 `U-109` 合并收**（同一根因的第二种伤害）。⚠️ **残余**：① 我只证了 `RESET` 这一条制造占位符的路径，别的会话清理路径未测，不打包票；② "P1 换 `'*'` 哨兵可顺手关掉本形态"目前是**推断、未实测**；③ 本轮先前那几个数（98,359 等）在它被解释之前**不得当作任何其他命题的证据**用 —— 我先前还用它"排除"过一个其实没排除的假设（`RELAY.md` §十六①）|
