...
| **R-17 未解决风险：`RESET` 占位符 GUC 在并行计划下静默少算**（`U-110`，已并入 `U-109`；**架构 v1.5 把风险面从「计数不可信」上调为「结果正确性」**，修复不在我手里） | 同会话·同 REPEATABLE READ 快照·每轮只切换并行度，4 轮全复现：**串行恒 200,000 / 并行 119,181、104,708、93,019、115,622（每次不同、全部偏小、无报错）**，属主真值同轮取；W6 独立复现到同一族数（100,385 / 97,282 / 103,929）。⇒ **现象与影响面成立**；⚠️ 但**机制仍未定死**：我给的「占位符不进 worker」这条解释，其关闭证据（同一 run 同源三件套）因本机 Docker 引擎中断**未跑**，已落成入库脚本 `deploy/loadtest/rls_parallel_evidence.sql` 等环境恢复；同时我对 W6 那条「worker 看得见值」的反证提了**同一个提升性问题**（不含列引用的表达式可由 leader 在 Gather 之上算完）⇒ 两侧都不许在机制上下结论。另撤回两条我先前当证据用的推导（`RELAY.md` §十八①：`bind.py:194` 的 None 来源、`stage=intent` 的读法方向 —— 后者改的是我自己量具的语义，已写进 `driver.py:_provenance`）。⚠️ **残余**：① 只证了 `RESET` 这一条制造占位符的路径，别的会话清理路径未测；② 生产注入形态（`set_config(...,true)`）W4/W6 读数恒精确，但仍待同一 run 覆盖；③ 这些并行数在被解释之前**不得当作任何其他命题的证据**用 |
