...
| **W7 自家缺陷：台账少计"被取消但上游已返回 200"的调用**（09-21） | 同一份容器日志的 httpx 200 计数 vs `llm_call` 事件计数 vs `app.cost_ledger` 行数（15 / 10 / 10） | ❌ **响应从未进入记账路径 ⇒ 按调用次数台账低 33%（5/15），按金额不可知**。影响我两条已承诺的口径：**成本告警/日报**、**跑批前报价**（真实上游花费 > 台账花费）。⚠️ **不并案**：与 U-116（可读性）、与 `bind` 超时是三件事，同一次读数里撞见；归号请架构裁，**我不自开 U-xx** |
| **本轮暂不推送（一次共享状态动作的停顿，不是故障）** | `git branch -r --contains a7e31b6` 返回空 + `git rev-list --left-right --count origin/main...main` = `0 2` + 门禁测试我自己跑（**1 failed / 7 passed**） | ⚠️ 待推的两个本地 commit 里含 W2B 的 `docs(w2b)`，其 `backend/reports/w2b/RELAY.md:391`（+ 未跟踪 `.bak-20260921-1631`）被 `test_migration_dsn_hygiene` 命中 ⇒ **我一推就把别人的红推上 main**。命中的是**占位串** `postgresql+psycopg://` + `user:pass@` 形态（W2B 在写"零命中"时原样引用了模式串本身）⇒ **无凭据泄露、`sk-` 未命中**。⚠️ **并且我自己刚踩了同一个坑**：为描述这条红，我把模式串原样写进了本行与 `RELAY.md` §二十五④(c)，门禁立刻新增两处命中（复跑 `1 failed / 7 passed`，violations 里出现 `w7/DELIVERY.md:136`、`w7/RELAY.md:1473`）⇒ 已拆成两段代码跨度。**这不是我粗心，是这个门禁的必然产物：任何引用规则自身的报告都会红，包括报告这个规则的报告。** ⇒ 修法只有两条且都不在我手里：W2B 改那一行并删 `.bak`，或架构裁"门禁不该扫 `reports/**`" |
