...
| **并行计划下 `count(*)`/聚合静默偏小**（回答 W6 `P6` 时撞见，**不是我的代码**） | 现象可复现：`app.shop_ids` 被 `RESET` 成 `''` 后，同一条 `count(*) from app.v_order_paid` **串行 200,000 / 并行 98,359、98,052、1,271**（三次三个数、全部偏小、无报错）。**机制未定位** ⇒ 已排除两条假设：① 不是"占位符 GUC 不传给 worker"（与 RLS 无关的 `app.canary` 谓词在 `traffic_daily` 上并行/串行都是 600,178，分毫不差）；② 不是数据在变。⇒ 对 W6 下一步"把 PG 执行链接进评测"（G-4/N-07）**直接相关**：被并行计划吃掉的分量不会报错。建议那条链上做一次"并行开/关结果相等"的对照 |
